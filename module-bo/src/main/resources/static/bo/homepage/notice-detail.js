document.addEventListener("DOMContentLoaded", function () {
    tinymce.init({
        ...
        init_instance_callback: function (editor) {
            const existingContent = $('input[name=description]').val();
    ...
    $('button[data-role=btnSave]').click(function () {
        ...
        const saveForm = {
            ...
            title: title,
            content: tinymce.get('content').getContent(), // 또는 $('#description').val()
            ...
            endDt: $('input[name=expireDate]').val().trim(),
            endNoLimitYn: $('input[name=noLimit]').is(':checked') ? 'Y' : 'N',
            popupYn: $('select[name=popupUseYn]').val()
        };

        $.ajax({
            type: "POST",
            url: `/bo/homepage/notice/update`,
